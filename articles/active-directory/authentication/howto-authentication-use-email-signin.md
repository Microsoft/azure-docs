...
* Some flows are currently not compatible with non-UPN emails, such as the following:
    ...
    * When a user is signed-in with a non-UPN email, they cannot change their password. This is different from Azure AD self-service password reset (SSPR), which should work as expected.    
